...
public:
	...
	void setStartConcToSteadyState(juce::OwnedArray<SimEntity> &, int idSS, const bool useEpsilonInsteadOfZero = false); // set concentrations to Steady State
	...
	public:
		...
		SimulationEvent(Type t,
						...
						juce::Array<float> entityGillespievalues = juce::Array<float>(),
			...
			: type(t), sim(sim), redrawPatch(_redrawPatch), redrawRun(_redrawRun), run(_run), nStep(nStep), currentTime(_currentTime), entityValues(entityValues), entityGillespievalues(entityGillespievalues), entityColors(entityColors), PACsValues(PACsValues), RACList(RACList)
		{
		}
