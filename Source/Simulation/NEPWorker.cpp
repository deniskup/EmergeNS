...
{
  if (nlsolverType == 0)
    results = findOptimalMomentumAndTime_brutforce(n, maxPrintingAllowed);
  else if (nlsolverType == 1)
    results = findOptimalMomentumAndTime_opt(n-1, maxPrintingAllowed);
  else if (nlsolverType == 2)
    results = findOptimalMomentumAndTime_LF(n-1, maxPrintingAllowed);
  else
    results = findOptimalMomentumAndTime_brutforce(n, maxPrintingAllowed);
...
dsp::Matrix<double> NEPWorker::buildOrthogonalBasis(StateVec v)
{
  int dim = v.size();
  if (dim == 0)
  {
...
gsl_vector * NEPWorker::initialOptimalGuess_brutforce(const int n, const bool gsl_status_previous_point, vector<double> previous_gsl_result, const StateVec qcenter)
{
  ...
  if (gsl_status_previous_point)
  {
    ...
    // dt = (dq•dH/dp) / || dH/dp ||^2
    StateVec dHdp = solver->evalHamiltonianGradientWithP(qcenter, pinit);
    double norm2 = 0.;
    double sp = 0.;
...
// gsl_status_previous_point : gsl status of the previous point, used as initial guess of set to true
gsl_vector * NEPWorker::initialOptimalGuess(const int n, const bool gsl_status_previous_point, vector<double> previous_gsl_result, const StateVec qcenter)
{
  gsl_vector* x = gsl_vector_alloc(n);
