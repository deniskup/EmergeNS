...
public:
    SATSolver(){};
    ~SATSolver(){};
    SATSolver(juce::String name, juce::String command, string satLine, bool printsExtraString): name(name), command(command) , satLine(satLine), printsExtraString(printsExtraString){};
    juce::String name;
    juce::String command;
...
public:
    PAC(){};
    PAC(juce::var data, Simulation *simul); // import from JSON, TODO
    ~PAC(){
        entities.clear();
        reacDirs.clear();
    };
    juce::var toJSONData();                 // save to JSON, TODO

    juce::String toString(); // for printing
    ...
    juce::Array<pair<SimReaction *, bool>> reacDirs; // direction 0 is 2->1 and 1 is 1->2
...
typedef juce::Array<pair<SimEntity *,float>> witnessType; // a witness is a vector of concentrations
typedef std::pair<set<int>,witnessType> CACType; // a CAC is a pair of a set of PACs and a witness

class PAClist : public juce::Thread
{
public:
    PAClist() : juce::Thread("PACs") {};
    PAClist(Simulation *simul) : juce::Thread("PACs"), simul(simul){};
    ~PAClist();

    Simulation *simul;
    juce::OwnedArray<PAC> cycles;
    juce::OwnedArray<PAC> nonMinimals;
    ...
    //CACs
    juce::Array<int> basicCACs; // indexes of the PACs in "cycles" that are basic CACs
    juce::Array<CACType> CACs; // indexes of the PACs in "cycles" that are CACs. each is a vector because we also treat pairs, etc.
    ...
    juce::String CACToString(CACType); // print CACs to string
    juce::var CACtoJSON(CACType); // save CACs to JSON
    CACType JSONtoCAC(juce::var); // load CACs from JSON
